...
{
  $hrurl = SeoText::getHrurlByUrl(sfContext::getInstance()->getController()->genUrl($internal_uri, $absolute));
  if ($hrurl && $hrurl != '') {
    $value = preg_split('/\?/', sfContext::getInstance()->getController()->genUrl($internal_uri, $absolute));
    return $value[0] . $hrurl . '/';
  }
