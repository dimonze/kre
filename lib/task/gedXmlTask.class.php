...
class gedXmlTask extends sfBaseTask
{
  protected
...
The [gedXml|INFO] task does things.
Call it with:

  [php symfony gedXml|INFO]
EOF;
        ...
        $w->startElement($container);
          ...
          foreach($ids as $id) {
            ...
            try
            {
              ...
              $w->endElement(); //offer
              $i['good'] ++;
              $this->logSection($id, sprintf('added. %s of %s (%s%%)', $i['good'], $i['total'], round($i['good'] / $i['total'] * 100, 2)
              ));
  ...
  protected function doCountry(XMLWriter $w, $lot)
  {
    ...
    $w->writeElement('district', $this->getWard($lot->ward));
    $this->getMetroDistance($w, $lot);
    $w->startElement('description');
    $w->writeCdata(exportBaseTask::getLotDescription($lot));
    $w->endElement();
    ...
    $w->startElement('areas');
    if((int)$lot->area_from > 0){
      $this->addArea($w, $lot);
    }
    if((int)$this->getAttr($lot->params, 'spaceplot') > 0){
      $w->startElement('area');
