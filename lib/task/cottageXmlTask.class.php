 ...
 * @author dimonze
 */
class cottageXmlTask extends sfBaseTask
{
  protected
  ...
  protected function configure()
  {
    ...
    $this->namespace        = '';
    $this->name             = 'cottageXml';
    $this->briefDescription = '';
    $this->detailedDescription = <<<EOF
The [cottageXml|INFO] task does things.
Call it with:

  [php symfony cottageXml|INFO]
EOF;
      ...
      if(is_callable(array($this, $method))) {
        ...
        $w->startElement('offers');
          ...
          foreach($ids as $id) {
            ...
            try {
              if(!$this->translateObjectAndPart($lot, 'object') || $lot->params['objecttype'] == 'Коттеджный поселок') {
  ...
  {
    $w->writeElement('type', in_array($lot->type, Lot::$_statuses_rent) ? 'аренда' : 'продажа');
    $w->writeElement('property-type', 'жилая');
    ...
    if ($lot->type == 'outoftown' || $lot->type == 'cottage') {
    $w->writeElement('parent-object-name', $lot->params['cottageVillage']);
    }else {
    $w->writeElement('parent-object-name', $lot->params['estate']);
    }
    $w->startElement('location');
      $w->writeElement('region', 'Москва');
      $w->writeElement('sub-locality-name', $this->translateDistrict($lot));
    ...
    $w->writeElement('parking', (boolean)($this->getAttr($lot->params, 'parking')));
//    $w->startElement('preview');
//      $w->writeCdata($lot->anons);
  ...
  protected function doNewbuilds(XMLWriter $w, $lot)
  {
    ...
    $w->startElement('location');
      ...
      $w->startElement('metro');
        $w->writeAttribute('name', $this->getMetro($lot->metro_id));
...
//    $w->endElement(); //preview
  ...
  protected function doCountry(XMLWriter $w, $lot)
  {
    ...
    $w->startElement('location');
      ...
      $w->writeElement('locality-name', $lot->address['city']);
      $w->writeElement('distance', $this->getAttr($lot->params, 'distance_mkad'));
      $w->writeElement('direction',  $this->getWard($lot->ward).' шоссе');
      $w->writeElement('latitude', $lot->lat);
      $w->writeElement('longitude', $lot->lng);
    $w->endElement(); //location
    $w->startElement('sales-agent');
    ...
    $w->writeElement('value', $lot->area_from);
     ...
     $w->writeElement('period', 'месяц');
    $w->endElement();// area
    $w->startElement('lot-area');
    $w->writeElement('value', (int)$this->getAttr($lot->params, 'spaceplot'));
     $w->writeElement('unit', 'сот');
    $w->endElement();// lot-area
//    $w->startElement('preview');
//      $w->writeCdata($lot->anons);
//    $w->endElement(); //preview
    $w->startElement('description');
