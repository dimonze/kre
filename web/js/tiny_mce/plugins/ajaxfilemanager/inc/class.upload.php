...
class Upload
{
	...
	function moveUploadedFile($dest, $fileBaseName = '', $overwrite=false)
	{
		...
		$dotIndex = mb_strrpos($fileName, '.');
		$this->fileExtension = '';
		if(is_int($dotIndex))
		{
			$this->fileExtension = mb_substr($fileName, $dotIndex);
			$this->fileBaseName = mb_substr($fileName, 0, $dotIndex);
		}
		if(!empty($fileBaseName))
		{
			$this->fileBaseName = $fileBaseName;
		}
    $this->fileBaseName = slugify($this->fileBaseName);
