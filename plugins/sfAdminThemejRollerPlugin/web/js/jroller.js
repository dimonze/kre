jQuery().ready(function(){
  ...
  if (!$('#sf_admin_actions_menu_list').children().length) {
    $('#sf_admin_actions_button').remove();
  }
  ...
  //save form by click on save button in footer
  $('.sf_admin_action_save button, .sf_admin_action_save_and_add button', '#sf_admin_footer').click(function(){
    $('.sf_admin_form form').submit();
  });

  //submit batch actions on top
  $('.sf_admin_actions_top').find('input[type=submit]').click(function(){
    $('ul.sf_admin_actions', 'form').find('select').val($(this).prevAll('select').val());
    $('#sf_admin_content').children('form').submit();
  });
