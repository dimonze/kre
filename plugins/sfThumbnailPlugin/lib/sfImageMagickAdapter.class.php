...
class sfImageMagickAdapter
{
  ...
  public function save($thumbnail, $thumbDest, $targetMime = null)
  {
    ...
    switch (@$this->options['method'])
    {
      ...
      case 'shave_bottom':
        $width = $thumbnail->getThumbWidth();
        $height = $thumbnail->getThumbHeight();

        if (is_null($thumbDest))
        {
          $command = sprintf(
            " -resize %dx%d^ -gravity north -extent %dx%d %s '-' | %s",
        ...
        {
          $command = sprintf(
            " -resize %dx%d^ -gravity north -extent %dx%d %s %s && %s",
            $width, $height, $width, $height,
    ...
    {
      $do_wm_command = 'composite -dissolve 10% -tile %s %s %s';
      $command2 = 'convert %s -resize %d miff:- | composite -gravity center - %s %s';
    }
    ...
    if(isset($do_wm_command))
    {
      $cmd = sprintf($command2, $this->options['watermark'], $thumbnail->getThumbWidth(), escapeshellarg($output), escapeshellarg($output));
      (is_null($thumbDest))?passthru($cmd):exec($cmd);
    }
  }
